...
namespace muuk {
    ...
    void TomlHandler::set_section(const std::string& name, const toml::table& table) {
        auto it = sections.find(name);
        if (it != sections.end()) {
    ...
    void TomlHandler::set_section(const std::string& name, const std::vector<toml::table>& tables) {
        auto it = sections.find(name);
        if (it != sections.end()) {
            if (it->second.is_array()) {
    ...
    void TomlHandler::serialize_table(const toml::table& table, std::stringstream& output, int indent_level) const {
